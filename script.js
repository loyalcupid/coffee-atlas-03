/* ================================================================
   ...
   Single-file SPA: router + Firebase Realtime DB + Storage
================================================================ */
...
/* Firebase snapshot → 배열 변환 */
function snapToArray(snapshot) {
  if (!snapshot.exists()) return [];
  return Object.entries(snapshot.val()).map(([id, data]) => ({
    id,
    ...data,
    atmosphere_images: toArr(data.atmosphere_images),
  }));
}

/* Firebase 배열 quirk 정규화 (object → array) */
function toArr(val) {
  if (!val) return [];
...
async function seedIfEmpty() {
  const snap = await get(ref(database, 'records'));
  if (snap.exists()) return;
  for (const { id, ...data } of SEED.records) await set(ref(database, `records/${id}`), data);
  for (const { id, ...data } of SEED.visits)  await set(ref(database, `visits/${id}`), data);
  for (const { id, ...data } of SEED.orders)  await set(ref(database, `orders/${id}`), data);
}
...
const db = {
  async findAll(table) {
    return snapToArray(await get(ref(database, table)));
  },
  async findById(table, id) {
    const snap = await get(ref(database, `${table}/${id}`));
    if (!snap.exists()) return null;
    const data = snap.val();
    return { id, ...data, atmosphere_images: toArr(data.atmosphere_images) };
  },
  async findWhere(table, field, value) {
    return (await this.findAll(table)).filter(r => r[field] === value);
  },
  async insert(table, item) {
    const { id: _ignore, ...data } = item;
    const newRef = await push(ref(database, table), data);
    return { id: newRef.key, ...data };
  },
  async update(table, id, changes) {
    await update(ref(database, `${table}/${id}`), changes);
  },
...
async function route() {
  const [p0, p1, , p3] = parseHash(location.hash);
  ...
  if (!p0 || p0 === '') {
    ...
    await renderRecordDetail(p1);
  } else if (p0 === 'records' && p1 && p3) {
    showPage('order-detail');
    await renderOrderDetail(p1, p3);
...
const spinnerHTML = `<div class="spinner-wrap" style="min-height:200px;"><div class="spinner"></div></div>`;
...
async function renderRecords() {
  ...
  const [records, visits, orders] = await Promise.all([
    db.findAll('records'), db.findAll('visits'), db.findAll('orders'),
  ]);
  ...
  const draw = () => {
    ...
    const summaries = records.map(r => {
      ...
      const lo = rv[0] ? orders.find(o => o.visit_id === rv[0].id) : null;
      return { ...r, date: rv[0]?.date || '', drink: lo?.drink_name || '' };
    }).filter(r =>
      !term || r.name.toLowerCase().includes(term) || r.location.toLowerCase().includes(term)
    );

    if (summaries.length === 0) {
      list.innerHTML = `<div class="empty-state">
        <div class="empty-state-icon">${SVG.coffee}</div>
        <p class="empty-state-text">${term ? `"${esc(term)}" 검색 결과가 없습니다.` : '아직 기록이 없습니다. 첫 커피를 기록해보세요!'}</p>
    ...
    list.innerHTML = summaries.map(r => `
      <a href="#/records/${r.id}" class="record-card">
        <div class="record-card-left">
          ...
          <div>
            <div class="record-card-name">${esc(r.name)} <span class="rating-badge">${SVG.star} ${r.rating}</span></div>
...
async function renderRecordDetail(recordId) {
  ...
  const record = await db.findById('records', recordId);
  if (!record) { wrap.innerHTML = '<p style="padding:40px;text-align:center;color:var(--brown-50);">기록을 찾을 수 없습니다.</p>'; return; }

  const visits = (await db.findWhere('visits', 'record_id', recordId))
    .sort((a, b) => b.date > a.date ? 1 : -1);

  let selectedVisitId = visits[0]?.id || null;

  const getOrders = async () => selectedVisitId
    ? db.findWhere('orders', 'visit_id', selectedVisitId)
    : Promise.resolve([]);
  ...
  const renderOrders = async () => {
    ...
    if (!orders.length) return '<p style="color:var(--brown-30);font-size:13px;font-style:italic;padding:8px;">기록된 커피가 없습니다.</p>';
    ...
    return orders.map(o => `
      ...
      (total > 0 ? `<div class="total-amount-bar" style="margin-top:8px;">
        <span class="total-amount-label">이번 방문 커피구매 총액</span>
        <span class="total-amount-value">₩${total.toLocaleString()}</span>
      </div>` : '');
  };

  const renderStars = v => [1,2,3,4,5].map(n =>
    `<span class="star-display ${v >= n ? 'filled' : ''}">${SVG.star}</span>`).join('');

  const draw = async (editing = false) => {
    const ordersHTML = await renderOrders();
    wrap.innerHTML = `
      <div class="detail-card">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:16px;margin-bottom:8px;">
          <div style="flex:1;min-width:0;">
            ${editing
              ? `<input class="detail-name-input" id="edit-name" value="${esc(record.name)}">`
              : `<h2 class="detail-name">${esc(record.name)}</h2>`}
            <div class="detail-location">${SVG.mapPin}
              ${editing
                ? `<input id="edit-location" value="${esc(record.location)}">`
          ...
          <div style="display:flex;flex-direction:column;align-items:flex-end;gap:8px;flex-shrink:0;">
            <div style="display:flex;align-items:center;color:#d97706;font-weight:700;background:#fefce8;padding:6px 12px;border-radius:20px;border:1px solid #fef08a;gap:4px;">
              ${SVG.star}
              ${editing
                ? `<select id="edit-rating" style="background:transparent;border:none;outline:none;font-weight:700;font-size:16px;">${[1,2,3,4,5].map(n=>`<option value="${n}" ${record.rating===n?'selected':''}>${n}</option>`).join('')}</select>`
                : `<span style="font-size:18px;">${record.rating}</span>`}
            </div>
          ...
          <div class="visits-area" id="visits-area">
            ...
            ${visits.map(v => `
              <div class="visit-chip-wrap">
                <button class="visit-chip ${v.id === selectedVisitId ? 'active' : ''}" data-visit-id="${v.id}">${SVG.calendar} ${v.date}</button>
        ...
        <div style="margin-bottom:24px;">
          ...
          <div class="orders-area">
            ...
            <div id="orders-chips">${ordersHTML}</div>
          </div>
        </div>
        <hr class="divider">
        <div style="margin-bottom:24px;">
          ...
          <div class="img-grid" id="detail-img-grid">
            ${(record.atmosphere_images || []).map(url => `<div class="img-thumb"><img src="${esc(url)}" alt="분위기"></div>`).join('')}
            <div class="img-add-btn" id="detail-img-add-btn">${SVG.camera}<span id="detail-img-label">사진 추가</span></div>
          </div>
    ...
    document.getElementById('add-order-btn')?.addEventListener('click', async () => {
      ...
      document.getElementById('orders-chips').innerHTML = await renderOrders();
    });
    ...
    imgInput?.addEventListener('change', async e => {
      ...
      if (!file) return;
      const label = document.getElementById('detail-img-label');
      if (label) label.textContent = '업로드 중...';
      const path = `cafe-images/${recordId}/${Date.now()}_${file.name}`;
...
async function renderOrderDetail(recordId, orderId) {
  const wrap = document.getElementById('order-detail-content');
  wrap.innerHTML = spinnerHTML;

  const order = await db.findById('orders', orderId);
  if (!order) { wrap.innerHTML = '<p style="padding:40px;text-align:center;color:var(--brown-50);">주문 정보를 찾을 수 없습니다.</p>'; return; }
    ...
    <div class="coffee-card" style="background:rgba(255,255,255,0.8);backdrop-filter:blur(8px);padding:32px;">
      ...
      <div style="margin-bottom:24px;">
        ...
        <div class="rating-row" id="od-rating-row">
          ${[1,2,3,4,5].map(n => `<button class="rating-btn ${state.rating===n?'active':''}" data-rating="${n}">${SVG.starLg} ${n}</button>`).join('')}
        </div>
      </div>
      <div style="margin-bottom:24px;">
        ...
        ${[{key:'acidity',label:'Acidity (산미)'},{key:'body',label:'Body (바디감)'},{key:'sweetness',label:'Sweetness (단맛)'}].map(t => `
          <div style="margin-bottom:24px;">
            <div style="display:flex;justify-content:space-between;align-items:flex-end;margin-bottom:12px;">
              <span style="font-weight:700;color:var(--brown);">${t.label}</span>
              <span id="taste-label-${t.key}" style="font-size:12px;color:var(--brown-50);">${state[t.key]}/5</span>
            </div>
      ...
      <button class="btn-save-order" id="od-save-btn">${SVG.save} 수정 완료</button>
    </div>`;
  ...
  wrap.querySelectorAll('[data-taste]').forEach(row => {
    row.querySelectorAll('.taste-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        const lbl = document.getElementById(`taste-label-${row.dataset.taste}`);
...
function initAddRecord() {
  ...
  addRecordState = { coffeeOrders: [defaultCoffee()], cafeRating: 3, images: [] };
  ...
  renderStarPicker('cafe-rating-picker', 3, 'yellow', v => { addRecordState.cafeRating = v; });
  renderCoffeeOrders();
  ...
  document.getElementById('img-file-input').onchange = async e => {
    ...
    if (label) label.textContent = '업로드 중...';
    const path = `cafe-images/temp-${Date.now()}/${file.name}`;
    const url  = await imgStore.save(path, file);
  ...
  container.innerHTML = addRecordState.coffeeOrders.map((o, i) => `
    <div class="coffee-order-card" data-order-idx="${i}">
      <div class="coffee-order-header">
        ...
        ${addRecordState.coffeeOrders.length > 1 ? `<button type="button" class="btn-remove-coffee" data-remove-idx="${i}">${SVG.trash}</button>` : ''}
      </div>
      <div class="form-grid-2" style="margin-bottom:20px;">
        <div>
          <label class="field-label">${SVG.coffee} 커피 이름</label>
          <input class="form-input" data-field="drink" data-idx="${i}" type="text" placeholder="예: 아이스 아메리카노" value="${esc(o.drink)}">
        </div>
        <div>
          <label class="field-label"><span style="font-size:14px;font-weight:700;">₩</span> 가격</label>
          <input class="form-input" data-field="price" data-idx="${i}" type="number" placeholder="예: 5500" value="${o.price}" min="0">
        </div>
      </div>
      ...
      <div>
        ...
        <textarea class="form-textarea" data-field="memo" data-idx="${i}" placeholder="이 커피의 향, 맛, 느낌을 적어주세요." style="min-height:80px;">${esc(o.memo)}</textarea>
      </div>
    </div>`).join('');

  addRecordState.coffeeOrders.forEach((o, i) => {
    renderStarPicker(`coffee-rating-${i}`, o.coffeeRating, 'brown', v => { addRecordState.coffeeOrders[i].coffeeRating = v; });
  });

  container.querySelectorAll('[data-remove-idx]').forEach(btn => {
    btn.addEventListener('click', () => {
      addRecordState.coffeeOrders.splice(Number(btn.dataset.removeIdx), 1);
      renderCoffeeOrders(); updateTotalPrice();
    });
  });
...
function renderRangeSlider(label, field, idx, value) {
  return `<div class="range-wrap">
    ...
    <input type="range" min="1" max="5" step="1" value="${value}" data-field="${field}" data-idx="${idx}">
...
function renderStarPicker(containerId, current, theme, onChange) {
  ...
  if (!el) return;
  const cls = theme === 'yellow' ? 'active-yellow' : 'active-brown';
  el.innerHTML = [1,2,3,4,5].map(n =>
    `<button type="button" class="star-btn ${current >= n ? cls : ''}" data-val="${n}">${SVG.starLg}</button>`).join('');
  let val = current;
  el.querySelectorAll('.star-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      val = Number(btn.dataset.val); onChange(val);
      el.querySelectorAll('.star-btn').forEach((b, i) => b.classList.toggle(cls, i < val));
    });
  });
}
...
async function handleAddRecordSubmit(e) {
  ...
  if (addRecordState.coffeeOrders.some(o => !o.drink.trim())) {
    alert('모든 커피의 이름을 입력해주세요.'); return;
  }
  ...
  try {
    ...
    for (const o of addRecordState.coffeeOrders) {
    ...
    btn.disabled = false;
    btn.innerHTML = `${SVG.save} 기록 완료하기`;
  }
}
...
async function renderDashboard() {
  document.getElementById('dash-stats').innerHTML = spinnerHTML;
  ...
  const bestCafe = records.map(r => {
    ...
    return { ...r, avgRating: ords.length ? ords.reduce((s,o)=>s+(o.rating??0),0)/ords.length : 0, visitCount: (visitMap[r.id]||[]).length };
  ...
  const ratingLevel = avgRating>=4.5?'엄격한 미식가':avgRating>=3.5?'커피 애호가':avgRating>0?'커피 입문자':'기록 없음';

  document.getElementById('dash-subtitle').textContent = `총 ${totalCafes}개 카페 · ${totalVisits}번의 방문 기록`;
  ...
  document.getElementById('dash-body').innerHTML = `
    <div class="dash-row dash-row-2">
      <div class="dash-card-dark">
        ...
        <div class="taste-type-badge">
          ...
          <div><p class="taste-type-name">${tasteType.label}</p><p class="taste-type-desc">${tasteType.desc}</p></div>
        </div>
        <div class="taste-bars">
          ${[{label:'산미 (Acidity)',val:avgAcidity,color:'#7EC8E3'},{label:'바디감 (Body)',val:avgBody,color:'#C8A97E'},{label:'단맛 (Sweetness)',val:avgSweetness,color:'#D4AF37'}].map(t=>`
            <div class="taste-bar-row">
              <div class="taste-bar-header"><span class="taste-bar-label">${t.label}</span><span class="taste-bar-val">${t.val>0?t.val+' / 5':'-'}</span></div>
              <div class="taste-bar-track"><div class="taste-bar-fill" style="width:${(t.val/5)*100}%;background:${t.color};"></div></div>
            </div>`).join('')}
        </div>
      </div>
      <div style="display:flex;flex-direction:column;gap:16px;">
        ...
        <div class="dash-card" style="flex:1;">
          <div class="row-between" style="margin-bottom:16px;"><p class="dash-eyebrow" style="color:var(--brown-50);">자주 마신 음료</p><span style="color:var(--brown-30);">${SVG.coffee}</span></div>
          ${topDrinks.length===0
            ? `<p style="color:var(--brown-30);font-size:14px;text-align:center;padding:24px 0;">아직 기록이 없어요</p>`
            : `<div class="top-drinks">${topDrinks.map(([name,count],i)=>`
                <div class="top-drink-row">
                  <div class="top-drink-header"><span class="top-drink-name">${medals[i]||(i+1)+'.'} ${esc(name)}</span><span class="top-drink-count">${count}잔</span></div>
                  <div class="top-drink-track"><div class="top-drink-fill ${i===0?'gold':'brown'}" style="width:${(count/maxDrink)*100}%;"></div></div>
                </div>`).join('')}</div>`}
        </div>
      </div>
    </div>
    <div class="dash-row dash-row-2">
      <div class="dash-card">
        <div class="row-between" style="margin-bottom:24px;">
          <div><p class="dash-eyebrow" style="color:var(--brown-50);">방문 히스토리</p><h2 class="dash-title" style="color:var(--brown);">월별 카페 방문 횟수</h2></div>
          <div style="width:40px;height:40px;background:var(--cream);border-radius:12px;display:flex;align-items:center;justify-content:center;border:1px solid var(--brown-10);">${SVG.trending}</div>
        </div>
        <div class="monthly-bar-chart">
          ${monthlyData.map(d=>`
            <div class="month-bar-col">
              <span class="month-bar-count">${d.count}</span>
              <div class="month-bar-track"><div class="month-bar-fill" style="height:${d.count>0?Math.max((d.count/maxMonth)*100,8):3}%;background:${d.count>0?'linear-gradient(180deg,#D4AF37 0%,#3D2B1F 100%)':'rgba(61,43,31,0.08)'};border-radius:4px 4px 0 0;"></div></div>
              <span class="month-bar-label">${d.label}</span>
            </div>`).join('')}
        </div>
        <div class="row-between" style="font-size:12px;color:var(--brown-50);margin-top:16px;padding-top:16px;border-top:1px solid var(--brown-5);"><span>최근 6개월</span><span>총 ${visits.length}회 방문</span></div>
      </div>
      ${bestCafe
        ...
        : `<div class="dash-card" style="display:flex;align-items:center;justify-content:center;">
            <div class="no-best-cafe"><div class="no-best-icon">${SVG.award}</div><p class="no-best-text">카페를 기록하면<br>최고 평점 카페가 표시돼요</p><a href="#/add-record" class="no-best-link">첫 기록 남기기 →</a></div>
          </div>`}
    </div>
    <div class="dash-card">
      <div class="row-between" style="margin-bottom:20px;">
        <div><p class="dash-eyebrow" style="color:var(--brown-50);">Recent Activity</p><h2 class="dash-title" style="color:var(--brown);">최근 방문 기록</h2></div>
      ...
      ${recentVisits.length===0
        ? `<div style="text-align:center;padding:40px 0;">${SVG.coffee}<p style="color:var(--brown-30);font-size:14px;margin-top:12px;">아직 기록이 없습니다.</p><a href="#/add-record" style="font-size:14px;font-weight:700;color:var(--brown);display:inline-block;margin-top:12px;">첫 카페 기록하기 →</a></div>`
        : recentVisits.map(({visit,record,order})=>`
          <a href="#/records/${record.id}" class="recent-visit-item">
            <div class="visit-date-badge"><span class="visit-date-month">${visit.date?.slice(5,7)}월</span><span class="visit-date-day">${visit.date?.slice(8,10)}</span></div>
            <div class="visit-info">
              <p class="visit-cafe-name">${esc(record.name)}</p>
              <div class="visit-meta">${order?.drink_name?`<span class="visit-drink">${esc(order.drink_name)}</span><span>·</span>`:''} ${SVG.mapPin} <span>${esc(record.location)}</span></div>
            </div>
    ...
    <div class="dash-card">
      <div style="margin-bottom:20px;"><p class="dash-eyebrow" style="color:var(--brown-50);">Taste Deep Dive</p><h2 class="dash-title" style="color:var(--brown);">기록 기반 상세 통계</h2></div>
      <div class="taste-deep-grid">
        ${[
          ...
          <div class="taste-deep-card" style="background:${item.bg};">
            <div class="taste-deep-top"><div class="taste-deep-icon-wrap" style="background:${item.color}25;color:${item.color};"><span style="font-size:18px;">${item.icon}</span></div><span class="taste-deep-label">${item.label}</span></div>
            <p class="taste-deep-value">${item.avg>0?item.avg:'-'}${item.avg>0?`<span class="taste-deep-unit">/5</span>`:''}</p>
            <div class="taste-deep-segments">${[1,2,3,4,5].map(n=>`<div class="taste-deep-seg" style="background:${n<=item.avg?item.color:item.color+'25'};"></div>`).join('')}</div>
            <p class="taste-deep-sub">${orders.length?`${item.high}개 기록이 4점 이상`:'기록 없음'}</p>
          </div>`).join('')}
      </div>
      ${orders.length>0?`<div class="taste-summary-bar"><span class="taste-summary-emoji">${tasteType.emoji}</span><div><p class="taste-summary-text">${tasteType.label} — ${tasteType.desc}</p><p class="taste-summary-sub">총 ${orders.length}잔의 커피 기록을 바탕으로 분석한 결과입니다.</p></div></div>`:''}
    </div>`;
}

function getTasteType(acidity, body, sweetness) {
  if (!acidity && !body && !sweetness) return { emoji:'☕', label:'기록 중',       desc:'더 많은 커피를 기록해보세요' };
...
function buildRadarSVG(acidity, body, sweetness) {
  ...
  const axes = [{angle:-90,label:'산미',val:acidity},{angle:30,label:'바디감',val:body},{angle:150,label:'단맛',val:sweetness}];
  ...
  const grid = [0.2,0.4,0.6,0.8,1].map(s=>`<polygon points="${axisPts.map(p=>`${cx+(p.x-cx)*s},${cy+(p.y-cy)*s}`).join(' ')}" fill="none" stroke="rgba(255,255,255,0.12)" stroke-width="1"/>`).join('');
  return `<div class="taste-radar-wrap"><svg width="${W}" height="${H}" viewBox="0 0 ${W} ${H}">
    ${grid}
    ...
    ${axes.map((a,i)=>`<text x="${labelPts[i].x}" y="${labelPts[i].y-6}" text-anchor="middle" font-size="11" font-weight="700" fill="white">${a.label}</text><text x="${labelPts[i].x}" y="${labelPts[i].y+8}" text-anchor="middle" font-size="10" fill="#D4AF37" font-weight="600">${a.val.toFixed(1)}</text>`).join('')}
...
async function renderMap() {
  ...
  const draw = () => {
    ...
    const items = records.map(r => {
      ...
      const lo = rv[0] ? orders.find(o=>o.visit_id===rv[0].id) : null;
      return { ...r, date: rv[0]?.date||'', drink: lo?.drink_name||'' };
    }).filter(r => !term || r.name.toLowerCase().includes(term) || r.location.toLowerCase().includes(term));

    if (!items.length) {
      listEl.innerHTML = `<div class="empty-state"><div class="empty-state-icon">${SVG.coffee}</div>
        <p class="empty-state-text">${term?`"${esc(term)}" 검색 결과가 없습니다.`:'아직 기록이 없습니다.'}</p>
        ${!term?`<a href="#/add-record">첫 카페 기록하기 →</a>`:''}</div>`;
      return;
    }
    listEl.innerHTML = items.map(r=>`
      <div class="map-cafe-card" data-naver-query="${esc(r.name+' '+r.location)}">
        <div style="display:flex;align-items:center;gap:20px;">
          ...
          <div>
            <div class="map-cafe-name">${esc(r.name)} <span class="rating-badge">${SVG.star} ${r.rating}</span></div>
            ...
            <div style="display:flex;align-items:center;gap:12px;font-size:12px;color:var(--brown-50);margin-top:4px;">
              <span style="display:flex;align-items:center;gap:4px;">${SVG.mapPin} ${esc(r.location)}</span><span>•</span><span>${esc(r.date)}</span>
            </div>
          </div>
        </div>
        <div style="display:flex;align-items:center;gap:16px;"><span class="map-naver-link hide-sm">네이버 지도 ${SVG.external}</span>${SVG.chevron}</div>
      </div>`).join('');
