...
export default function NewExpertCafePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const newRef = await push(dbRef(db, "expertCafes"), {
        name: cafeName.trim(),
        photos: cafePhotos,
        barista,
      ...
      router.push(`/expert-tour/${newRef.key}`);
    } catch (err) {
      console.error(err);
      ...
      <div className="w-full border-b border-[#D4AF37]/20 px-6 py-5">
        <div className="max-w-3xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <span className="text-[#D4AF37]/30">/</span>
            <span className="cormorant text-[#FCF5E5]/60 text-base">카페 등록</span>
          </div>
          <Link href="/expert-tour" className="cormorant text-[#FCF5E5]/30 hover:text-[#FCF5E5]/60 transition-colors text-sm">
            취소
          </Link>
        </div>
      </div>
      ...
      <div className="max-w-3xl mx-auto px-6 py-10 space-y-6">
        ...
        <div className="text-center space-y-2 mb-8">
          <div className="gold-divider text-[#D4AF37]/50 text-xs tracking-[0.4em] uppercase cormorant">
            Register Expert Café
          </div>
          <h1 className="playfair text-3xl font-bold text-[#FCF5E5]">카페 등록하기</h1>
          <p className="cormorant text-[#FCF5E5]/40 text-lg font-light">
            당신이 사랑하는 커피 고수 카페를 소개해주세요.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          >
            {loading ? "저장 중..." : <><Send size={20} /> 카페 등록하기</>}
          </button>
        </form>
      </div>
